...
const WithdrawHistoryAllUser = require("../models/WithdrawHistoryAllUser");
//const UserCall = require('../models/UserCall'); 
...
exports.registerSubAdmin = async (req, res) => {
  try{
    ...
    res.status(200).json({message:'Admin registered Sucessfully',admin})
  }catch(error){
    res.status(500).json({message:'Internal Server Error'})
  }
};
...
exports.userRequestOTP = async (req, res) => {
    try {
        const { mobileNumber, email, googleId } = req.body;

        // Check if the mobile number or email is blocked
        const isBlocked = await DeletedUsers.findOne({
            $or: [{ mobileNumber }, { email }],
        });

        // Default initialCoin value
        let initialCoin = 0;
        const initialCoinDoc = await FreeCoin.findOne();
        const status = initialCoinDoc ? initialCoinDoc.status : false;

        if (initialCoinDoc && status === true && !isBlocked) {
            initialCoin = initialCoinDoc.freeCoinforNewUser;
        }

        // Define hardcoded mobile numbers and OTP
        const hardcodedNumbers = ['9876543211', '9092568089']; // Add your specific numbers here
        const hardcodedOTP = 123456;
        ...
        if (mobileNumber) {
            ...
            if (user) {
                // Existing user flow
                let otp = hardcodedNumbers.includes(mobileNumber)
                    ? hardcodedOTP
                    : (await sendOTPViaService(mobileNumber)).otp;
                ...
                // Add wallet and initialCoin if not blocked
                if (!isBlocked) {
                    const wallet = new Wallet({
                        userId: user._id,
                        balance: initialCoin,
                ...
                return res.status(200).send({
                    message: isBlocked
                        ? 'New user created successfully (blocked, no initial coins).'
                        : 'New user created successfully with initial coins.',
...
// Helper function to send OTP via external service
const sendOTPViaService = async (mobileNumber) => {
    const response = await axios.get(`http://2factor.in/API/V1/9e880f4a-7dc5-11ec-b9b5-0200cd936042/SMS/${mobileNumber}/AUTOGEN2`);
    console.log("OTP response----------", response.data);
    return { otp: parseInt(response.data.OTP, 10) };
};
...
//     // Respond with the user profile and follow counts
	...
	requireCoins:conversionData.coinsDeductedPerMinute,
     });
   } catch (error) {
...
exports.endCall = async (req, res) => {
  const { hostId, joinerId, channelName, callDurationMinutes } = req.body;
console.log("callDurationMinutes------",callDurationMinutes);
  if (!hostId || !joinerId || !channelName || !callDurationMinutes) {
    return res.status(400).json({ error: 'Missing required fields' });
  }
  ...
  try {
    ...
    // Fetch conversion details for coins and hearts (diamonds)
    const conversion = await Conversion.findOne();
console.log("conversion----------",conversion);
    if (!conversion) {
      return res.status(500).json({ error: 'Conversion details not found' });
    }
console.log("conversion.coinsDeductedPerMinute",conversion.coinsDeductedPerMinute,callDurationMinutes)
    // Calculate coins to deduct and hearts (diamonds) to credit
    const coinsToDeduct = conversion.coinsDeductedPerMinute * callDurationMinutes;
    const heartsToCredit = conversion.diamondsReceivedPerMinute * callDurationMinutes;

    // Fetch users (joiner and host)
    const joinerUser = await User.findById(joinerId);
    const hostUser = await User.findById(hostId);
...
console.log("joinerUser.profile.coin -------",joinerUser.profile.coin ,coinsToDeduct);
    // Check if the joiner has enough coins to deduct
    if (joinerUser.profile.coin < coinsToDeduct) {
      return res.status(400).json({ error: 'Not enough coins to complete the transaction' });
    }

    // Deduct coins from joiner
    joinerUser.profile.coin -= coinsToDeduct;

    // Credit hearts (diamonds) to host
    hostUser.profile.heartBalance += heartsToCredit;
    hostUser.profile.fullheartBalance += heartsToCredit;
    ...
    const joinerTransaction = new CoinTransactionHistory({
      ...
      description: `Coins spent for a ${callEntry.callType} call`,
      spendingType: callEntry.callType,
      heartsTransferred: heartsToCredit,
    ...
    res.status(200).json({
      message: 'Call ended successfully, coins deducted, hearts transferred, and transactions recorded',
      callEntry,
      joinerTransaction,
      hostTransaction,
    });
...
console.log("transactions----------------",transactions);
        ...
        //    const callDuration = transactions.callDurationMinutes;
...
const razorpay = new RazorPay({
    key_id: "rzp_test_PdLIBpKPvtii3D",
    key_secret: "uQpmdptN84tlMAHJsdQwzlTa",
  });
...
console.log("razorpay--------",razorpay,req.body)
console.log("razorpay----------",razorpay)
  ...
  try {
    ...
    const coinPackage = await CoinPackage.findById(packageId);
console.log("----------------coinPackage",coinPackage);
    if (!coinPackage || !coinPackage.status) {
      return res.status(404).json({ error: "Coin package not found or inactive" });
    }

    const options = {
      amount: coinPackage.rateInInr * 100, // Convert to smallest currency unit (e.g., paise)
      currency: "INR",
...
console.log("signature--------------",response)
    ...
    console.error("Error purchasing coin package:", err);
    res.status(500).json({ error: "An error occurred while purchasing the coin package" });
  }
};

exports.verifyAndSaveTransaction = async (req, res) => {
  const { orderId, paymentId,signature, userId, packageId } = req.body;
console.log("----------------enter tge verifyAndSaveTransaction------------",req.body)
...
console.log("existingTransaction--------------",existingTransaction)
    const transaction = new Transaction({
      ...
      status: "success",
      type: "payment",
      createdAt: momentTime.tz('Asia/Kolkata').toDate(),
    });

    await transaction.save();
console.log("transaction-----------------",transaction)
   const user = await User.findById(userId);
console.log("user----------",user.profile.coin);
//return
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    }
...
//      transaction,
      updatedCoinBalance: user.profile.coin,
    });
  } catch (err) {
...
exports.getHostedUsers = async (req, res) => {
    try {
        // Fetch users where status is explicitly not 'newUser' and the status field exists
        const hostedUsers = await User.find({
            status: { $ne: 'newUser', $exists: true }
        }).select('_id username mobileNumber createdAt updatedAt profile newUser status');

        if (!hostedUsers || hostedUsers.length === 0) {
            return res.status(404).json({ message: 'No hosted users found.' });
        }

        // Map through the users to match the desired response structure
        const usersWithDetails = hostedUsers.map(user => ({
            _id: user._id,
            username: user.username,
            mobileNumber: user.mobileNumber,
...
exports.deleteUser = async (req, res) => {
    try {
        ...
        // Check and delete the user from the User collection
        const deletedUser = await User.findByIdAndDelete(userId);
        ...
        // Delete all records in the userOneVsOneList collection with the userId
        const deletedUserOneVsOneList = await UserOneVsOneList.deleteMany({ userId });

        res.status(200).json({
            message: 'User and associated data deleted successfully',
            details: {
                deletedUser,
                deletedUserOneVsOneListCount: deletedUserOneVsOneList.deletedCount, // Number of records deleted
            },
        });
    } catch (error) {
...
exports.deleteUser = async (req, res) => {
    try {
        ...
        // Check if the mobileNumber already exists in DeletedUsers
        const existingDeletedUser = await DeletedUsers.findOne({ mobileNumber: user.mobileNumber });

        if (!existingDeletedUser) {
            // Insert into DeletedUsers only if the mobileNumber doesn't exist
            await DeletedUsers.create({
                mobileNumber: user.mobileNumber,
                email: user.profile.email,
            });
        ...
        res.status(200).json({
            ...
            details: {
                deletedUser,
                deletedUserOneVsOneListCount: deletedUserOneVsOneList.deletedCount,
            },
        });
    } catch (error) {
        console.error('Error deleting user and associated data:', error);
...
// Fetch coinsDeductedPerMinute from Conversion model
const conversion = await Conversion.findOne();
if (!conversion) {
  return res.status(500).json({ error: 'Conversion rate not found' });
}
const userCoins = userEntry.profile.coin;  
...
*/
if (hostEntry.callType === 'audio') {
  maxCallTimeMinutes = Math.floor(userCoins / conversion.coinsDeductedPerMinute);
} else if (hostEntry.callType === 'video') {
  maxCallTimeMinutes = Math.floor(userCoins / conversion.coinsDeductedPerMinute);
}

// Validate if the user has enough coins to start a call
if (userCoins < conversion.coinsDeductedPerMinute) {
  return res.status(400).json({
    message: 'You do not have enough balance to join the call.',
  });
}
...
notification: {
  ...
  hostUserFcmToken:hostFcmToken?String(hostFcmToken) :'',
},
...
const message = {
  token: hostFcmToken, // Target device token
  data: {
    title: 'Call Notification',
   body:` A new call in ${hostEntry.callType} type has been started. Channel: ${hostEntry.channelName}`,
    channelName: String(hostEntry.channelName),
    ...
    res.status(200).json({
      ...
      hostDetails: {
        ...
        channelName: hostEntry.channelName,
        token: hostEntry.token,
       joinedUserFcmToken: joinedUserFcmToken,
       hostUserFcmToken:hostFcmToken,
        appId: APP_ID,
      }
    });
...
exports.getLeaderBoard = async (req, res) => {
  try {
    // Fetch users sorted by heartBalance (diamonds balance) in descending order
    const leaderboard = await User.find()
      .sort({ 'profile.heartBalance': -1 }) // Sort by heartBalance, descending
      .limit(10) // Fetch top 10 users
      .select('username profile.image profile.heartBalance'); // Only select necessary fields

    // Check if all users have heartBalance = 0
    const allZeroHeartBalances = leaderboard.every(
      (user) => (user.profile.heartBalance || 0) === 0
    );

    if (allZeroHeartBalances) {
      return res.status(200).json({
        success: true,
    ...
    const formattedLeaderboard = leaderboard.map((user, index) => ({
      rank: index + 1,
      username: user.username,
      avatar: user.profile.image || null, // Ensure avatar is null if not available
      heartBalance: user.profile.heartBalance || 0, // Ensure heartBalance is 0 if not available
    }));
...
exports.getMoneyPage = async (req, res) => {
  ...
  try {
    ...
    const money = user.profile.heartBalance* conversionRate.costPerHeart;

    res.status(200).json({
      success: true,
      message: 'Money page details fetched successfully',
      data: {
        heartBalance: user.profile.heartBalance,
...
exports.getMoneyPage = async (req, res) => {
  ...
  try {
    // Fetch the user's heart balance
    const user = await User.findById(userId).select('profile.heartBalance');
    if (!user) {
      return res.status(404).json({ success: false, message: 'User not found' });
    }

    // Fetch the diamond conversion factor
    const conversion = await Conversion.findOne(); // Fetch from the `conversion` collection
    if (!conversion || !conversion.diamondConversionFactor) {
      return res
        .status(500)
        .json({ success: false, message: 'Conversion factor not set' });
    }

    // Extract the diamond to rupee conversion factor
    const { diamond, rupee } = conversion.diamondConversionFactor;
    if (!diamond || !rupee) {
      return res
        .status(500)
        .json({ success: false, message: 'Invalid conversion factor' });
    }

    // Calculate the money equivalent
    const money = (user.profile.heartBalance / diamond) * rupee;
    ...
    res.status(200).json({
      ...
      data: {
        heartBalance: user.profile.heartBalance,
        conversionFactor: { diamond, rupee },
        totalMoney: money.toFixed(2),
      },
    });
  } catch (error) {
...
exports.redeemHearts = async (req, res) => {
  ...
  try {
    ...
    if (!user || user.profile.fullheartBalance <= 0) {
      return res
        .status(400)
        .json({ success: false, message: 'Insufficient hearts to redeem' });
    }
    ...
    // Calculate redeemable money
    const redeemableMoney = (user.profile.fullheartBalance / diamond) * rupee;

    // Update user's heart balance and reset it to zero
    const redeemedHearts = user.profile.fullheartBalance; // Store redeemed hearts before resetting
//    user.profile.heartBalance = 0;
    user.profile.fullheartBalance = 0; // Reset heart balance
    await user.save();
    ...
    res.status(200).json({
      ...
      message: 'Hearts redeemed successfully',
      data: {
        redeemedAmount: redeemableMoney.toFixed(2),
        walletBalance: (wallet.redeemedAmount - wallet.withdrawnAmount).toFixed(2),
      },
    });
  } catch (error) {
    console.error(error);
...
exports.createConversionFactor = async (req, res) => {
  ...
  try {
    // Create a new conversion factor entry
...
exports.updateConversionFactor = async (req, res) => {
  ...
  try {
    ...
    // Find the conversion factor by ID and update only provided fields
    const updatedConversion = await Conversion.findByIdAndUpdate(
      id,
      updateFields, // Only update the fields present in the request
      { new: true } // Return the updated document
    );
...
exports.getCoinPackagePurchases = async (req, res) => {
  ...
  try {
    ...
    if (filter === "week") {
      ...
      const startOfLastMonth = qmoment().subtract(1, "month").startOf("month").toDate();
...
exports.uploadKYCImage = async (req, res) => {
    try {
        const { userId, panNumber, nameOnPan, dob, aadhaarNumber, accountHolderName, bankName, accountNumber, reenterAccountNumber, ifcCode } = req.body;
...
//           adminRemarks:'Waiting for approval',
        });
...
console.log("userId--------",userId)
    ...
    if (kycRecord && kycRecord.kycStatus === 'approved') {
      return res.status(200).json({ status:kycRecord.kycStatus, success: true, flag: true });
    } else {
      return res.status(200).json({ success: true, flag: false });
    }
  } catch (error) {
...
console.log( "new Date().toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }))",new Date().toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }))
    if (kycRecord) {
      // If the record is found, return its status
      return res.status(200).json({ 
        status: kycRecord.kycStatus, 
        success: true, 
        flag: kycRecord.kycStatus === 'approved' 
      });
    } else {
      // If no record is found, return 'declined' as the status
      return res.status(200).json({ 
        status: 'declined', 
        success: true, 
        flag: false 
...
exports.updateKYC = async (req, res) => {
    try {
        ...
        // Check if userId is already an ObjectId, if not, convert it
        ...
        // Find the KYC record by userId
        const kycRecord = await KYC.findOne({ _id: userObjectId });
        ...
        res.status(200).json({ message: `KYC record ${kycStatus} successfully` });
    } catch (error) {
...
try{
    ...
    // Update .env file
    const newEnv = `
DATABASE =mongodb+srv://LeoDatingApp:EbED6jKLUwYJ34EW@leodatingapp.c4opg.mongodb.net/
RAZORPAY_KEY_ID=${RAZORPAY_KEY_ID}
RAZORPAY_KEY_SECRET=${RAZORPAY_KEY_SECRET}
`;

    fs.writeFileSync('.env', newEnv.trim(), { encoding: 'utf8' });
...
`;
        ...
        res.status(200).json({ message: 'Settings updated successfully. Server restart in progress.' });

        // Restart the PM2 process asynchronously
        exec('pm2 restart 0', (error, stdout, stderr) => {
            ...
            console.error('PM2 Restart Errors (if any):', stderr);
        });
    } catch (error) {
...
exports.CallTransactionHistory= async (req, res) => {
    try {
        // Fetch transactions in descending order by timestamp
        const transactions = await CoinTransactionHistory.find({})
            .sort({ timestamp: -1 })
            .lean();

        // Map through transactions to add user details
        const results = await Promise.all(
            transactions.map(async (transaction) => {
                // Fetch details for fromUserId and toUserId
                const fromUser = await User.findById(transaction.fromUserId).lean();
                const toUser = await User.findById(transaction.toUserId).lean();
console.log("------------fromUser---",fromUser); console.log("---------toUser------",toUser)
                return {
                    transactionId: transaction._id,
                    caller: {
                        userId: fromUser?._id || null,
                        username: fromUser?.username || null,
                        profileImage: fromUser?.profile?.image || null,
                    },
                    host: {
                        userId: toUser?._id || null,
                        username: toUser?.username || null,
                        profileImage: toUser?.profile?.image || null,
                    },
                    time: transaction.timestamp,
                    duration: transaction.callDurationMinutes,
                    coinDeducted: transaction.coins,
                    heartsTransferred: transaction.heartsTransferred,
...
try {
  const { userId, amount, detectAmount } = req.body;
  ...
  // Fetch user details to get userName and profileImage
  const user = await User.findOne({ _id: userId });
  if (!user) {
    return res.status(404).json({ success: false, message: "User not found" });
  }

  const userName = user.username || "Unknown User"; // Provide fallback value
  const profileImage = user.profile?.image || "default_image_url"; // Safe access with fallback value

  // Fetch WithdrawWallet Data
  const wallet = await WithdrawWallet.findOne({ userId });
  if (!wallet || wallet.redeemedAmount < amount) {
    return res.status(400).json({ success: false, message: "Insufficient balance or wallet not found" });
  }

  // Deduct amount and save wallet
  wallet.redeemedAmount -= amount;
  await wallet.save();

  // Save to WithdrawHistoryAllUser
  const history = new WithdrawHistoryAllUser({
    userId,
    userName,
    profileImage,
    amount,
  ...
  return res.status(200).json({
    success: true,
    message: "Withdrawal successful and logged",
    balance: wallet.redeemedAmount,
  });
} catch (error) {
...
exports.getAllWithdrawHistory = async (req, res) => {
  try {
    const { status } = req.query; // Fetch status from query params

    // Query object to dynamically filter based on status
    const query = {};
    if (status) {
      query.status = status; // Add status to query if provided
    }

    // Fetch filtered histories from the database
    const histories = await WithdrawHistoryAllUser.find(query);
...
exports.getWithdrawHistoryByUserId = async (req, res) => {
  try {
    const { userId } = req.params;
    ...
    const histories = await WithdrawHistoryAllUser.find({ userId });
    return res.status(200).json({ success: true, data: histories });
  } catch (error) {
    console.error("Fetch history by userId error:", error.message);
    return res.status(500).json({ success: false, message: "Server error", error: error.message });
  }
};
...
exports.updateWithdrawStatus = async (req, res) => {
  try {
    const { id } = req.params; // The ID of the withdrawal record
    ...
    const updatedRecord = await WithdrawHistoryAllUser.findByIdAndUpdate(
      id,
      { status },
      { new: true } // Return the updated document
    );
...
exports.exportWithdrawals = async (req, res) => {
  try {
    ...
    const { status } = req.query;
    if (!status) {
      return res.status(400).json({ success: false, message: "Status is required in query params" });
    }

    // Fetch withdrawals based on status
    const withdrawals = await WithdrawHistoryAllUser.find({ status });

    if (!withdrawals || withdrawals.length === 0) {
      return res.status(404).json({ success: false, message: `No withdrawals found for status: ${status}` });
    }

    // Fetch KYC data for all users involved in withdrawals
    const userIds = withdrawals.map((withdraw) => withdraw.userId);
    const kycData = await KYC.find({ userId: { $in: userIds } });
    const usersData = await User.find({ _id: { $in: userIds } }).select("mobileNumber email");

    // Create mappings for quick lookup
    const kycMap = {};
    kycData.forEach((kyc) => {
      kycMap[kyc.userId.toString()] = kyc.bankaccount;
    });

    const userMap = {};
    usersData.forEach((user) => {
      userMap[user._id.toString()] = { mobileNumber: user.mobileNumber, email: user.email };
    });
    ...
    // Define headers
    worksheet.columns = [
      { header: "Beneficiary Name (Mandatory) Special characters not supported", key: "beneficiaryName", width: 30 },
      { header: "Beneficiary's Account Number (Mandatory) Typically 9-18 digits", key: "accountNumber", width: 30 },
      { header: "IFSC Code (Mandatory) 11 digit code of the beneficiary’s bank account. Eg. HDFC0004277", key: "ifscCode", width: 20 },
      { header: "Payout Amount (Mandatory) Amount should be in rupees", key: "payoutAmount", width: 15 },
      { header: "Payout Mode (Mandatory) Select IMPS/NEFT/RTGS", key: "payoutMode", width: 15 },
      { header: "Payout Narration (Optional) Will appear on bank statement (max 30 char with no special characters)", key: "payoutNarration", width: 30 },
      { header: "Notes (Optional) A note for internal reference", key: "notes", width: 30 },
      { header: "Phone Number (Optional)", key: "phoneNumber", width: 15 },
      { header: "Email ID (Optional)", key: "email", width: 25 },
      { header: "Contact Reference ID (Optional) Eg: Employee ID or Customer ID", key: "contactRefId", width: 20 },
      { header: "Payout Reference ID (Optional) Eg: Bill no or Invoice No or Pay ID", key: "payoutRefId", width: 20 },
    ];

    // Populate rows with data
    withdrawals.forEach((withdraw) => {
      const kyc = kycMap[withdraw.userId.toString()] || {};
      const user = userMap[withdraw.userId.toString()] || {};

      worksheet.addRow({
        beneficiaryName: kyc.accountHolderName || "N/A",
        accountNumber: kyc.accountNumber || "N/A",
        ifscCode: kyc.ifcCode || "N/A",
        payoutAmount: withdraw.detectAmount || 0,
        payoutMode: "IMPS", // Example payout mode
        payoutNarration: "Withdrawal Payout",
        notes: `Withdrawal for user ${withdraw.userId}`,
        phoneNumber: user.mobileNumber || "N/A",
        email: user.email || "N/A",
        contactRefId: "", // Optional field
        payoutRefId: withdraw._id, // Example reference ID
      });
    });

    // Generate file path
//    const fileName = `Withdrawals_${status}_${Date.now()}.xlsx`;
  //  const filePath = path.join("/etc/ec/data/excel_withdraw/", fileName);

 // Generate file path with formatted date
    const now = new Date();
    const formattedDate = new Intl.DateTimeFormat("en-GB", {
      weekday: "short",
      month: "short",
      day: "numeric",
    }).format(now); // e.g., "Tue_Dec_18"
    const sanitizedDate = formattedDate.replace(/\s+/g, "_"); // Replace spaces with underscores
    const fileName = `Withdrawals_${status}_${sanitizedDate}.xlsx`;
    const filePath = path.join("/etc/ec/data/excel_withdraw/", fileName);
    ...
    const fileUrl = `https://leodatingapp.aindriya.uk/excel_withdraw/${fileName}`;
    res.status(200).json({ success: true, message: "File generated successfully", fileUrl });
  } catch (error) {
...
const generateToken = (uniqueChannelName) => {
  return RtcTokenBuilder.buildTokenWithUid(
    APP_ID,
    APP_CERTIFICATE,
    uniqueChannelName,
    0, // uid can be set to 0 for token generation without a specific user
    RtcRole.PUBLISHER,
//    Math.floor(Date.now() / 1000) + EXPIRATION_TIME_IN_SECONDS
    Math.floor(Date.now() / 1000) + (10 * 365 * 24 * 60 * 60)
  );
...
// Task to regenerate tokens
const regenerateTokens = async () => {
  try {
    console.log('Regenerating tokens...');

    // Fetch all entries in the collection
    const entries = await UserOneVsOneList.find();

    const updates = entries.map(async (entry) => {
      const user = await User.findOne({ _id: entry.userId });

      if (!user) {
        console.warn(`User not found for userId: ${entry.userId}`);
        return;
      }

      const uniqueChannelName = entry.channelName || `channel_${entry.userId}`;
      const newToken = generateToken(uniqueChannelName);
